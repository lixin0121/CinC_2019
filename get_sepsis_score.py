...
def get_sepsis_score(data,model):
    feature_select=[0,1,2,3,4,5,6,7,21,35,39];
...
#    M0=M0[:,feature_select]
    for j in range(0,len(M0)):
        if j<12:
            temp=np.zeros((12,11));
            temp[np.maximum(j-12,0):j,:]=M0[np.maximum(j-12,0):j,feature_select]; 
        else:
            temp=np.zeros((12,11));
            temp=M0[np.maximum(j-12,0):j,feature_select];
    X = temp;
...
def load_sepsis_model():
    model = load_model('my_model_RNN_v2.h5')
    return model